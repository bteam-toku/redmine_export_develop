...
def main():
    ...
    if len(holidaylist) > 0:
        ...
        calendar = CommonCalendar(temp_list)
    else:
    ...
    accessor = RedmineAccessor(project_name, its_url, its_key)
    # Issue情報を取得
    issues = accessor.load_issues()
    issues_num = len(issues)
    issue_update = accessor.latest_update()
    id_subject_dict = accessor.idtosubject_dict()
    ...
    exporter = RedmineExport(accessor)
    analyzer = RedmineAnalyze(accessor)
    # Information設定
    exporter.set_information(root, project_name, issue_update) # redmineExportのInformation設定
    # Header設定
    element = SubElement(root, "header")
    exporter.set_header(element) # redmineExportのHeader設定
    analyzer.set_header(element) # redmineAnalyzeのHeader設定
    # Body設定
    for issue in issues:
        element = SubElement(root, "ticket")
        element.set("id", str(issue.id))
        exporter.set_body(element, issue, id_subject_dict) # redmineExportのBody設定
        analyzer.set_body(element, issue, id_subject_dict, base_date, calendar) # redmineAnalyzeのBody設定
    ...
    common_xml.save_xml(root, xml_filepath)
    # csvファイル出力
    outfile_name = pathlib.Path(xml_filepath).with_suffix('.csv')
    common_xml.save_csv(root, str(outfile_name), 'header', 'ticket')
